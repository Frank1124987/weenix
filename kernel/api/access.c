...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        // first check if vaddr belongs to p
        vmarea_t* vma;
        ...
        // vma exist, proceed to check permission
        if (perm & PROT_EXEC){
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        uint32_t lastvfn = ADDR_TO_PN((char*)avaddr + len - 1);
        
        for (uint32_t vfn = startvfn ; vfn <= lastvfn ; vfn++){
