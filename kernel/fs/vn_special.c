...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
        ...
        int err = file->vn_cdev->cd_ops->read(file->vn_cdev, offset, buf, count);
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
        ...
        int err = file->vn_cdev->cd_ops->write(file->vn_cdev, offset, buf, count);
...
{
        int err = file->vn_cdev->cd_ops->mmap(file, vma, ret);
...
{
        int err = file->vn_cdev->cd_ops->fillpage(file, offset, pagebuf);
...
{
        int err = file->vn_cdev->cd_ops->dirtypage(file, offset);
...
{
        int err = file->vn_cdev->cd_ops->cleanpage(file, offset, pagebuf);
