...
sched_cancellable_sleep_on(ktqueue_t *q)
{
        ...
        /* when a thread is cancelled while sleeping, it will eventually wake up and KT_RUN. So we have to make sure it actually got cancelled */ 
        if (curthr->kt_cancelled == 1){
                /* sleep wake up because of cancel interrupt */
                dbg(DBG_PRINT, "(GRADING1C)\n");
                return -EINTR;
        }
...
{       
        ...
        /* how to make the thread run but curthr return and sleep ? -> by do_waitpid */
        intr_setipl(oldIPL);
