...
sys_read(read_args_t *arg)
{
        ...
        buf = kmalloc(kern_args.nbytes);
        ret = do_read(kern_args.fd, (void*) buf, kern_args.nbytes);
        ...
        if (ret < 0) {
                ...
                int err = copy_to_user(kern_args.buf, buf, ret);
                if (err < 0){
                        kfree((void*) buf);
                        curthr->kt_errno = err;
                        return -1;    
...
sys_write(write_args_t *arg)
{
        ...
        argstr_t* temp =  (argstr_t*) kmalloc(sizeof(argstr_t));
        temp->as_len = kern_args.nbytes;
        temp->as_str = (char*)kern_args.buf;
        if ((buf = user_strdup(temp)) == NULL) {
                kfree((void*) temp);
        ...
        ret = do_write(kern_args.fd, (void*) buf, kern_args.nbytes);
...
sys_getdents(getdents_args_t *arg)
{
        ...
        dirent_t* ent = (dirent_t*) kmalloc(sizeof(dirent_t));
        size_t max_count = kern_args.count / sizeof(dirent_t);

        int read;
        for (size_t i=0 ; i<max_count; i++){
                int cur_read = do_getdent(kern_args.fd, ent);
                if (cur_read < 0 ){
                        kfree((void*) ent);
                        curthr->kt_errno = -cur_read;
                        return -1; 
                }
                ...
                int err = copy_to_user(uaddr, ent, cur_read);
                if (err < 0){
                        kfree((void*) ent);
                        curthr->kt_errno = -err;
                        return -1;
